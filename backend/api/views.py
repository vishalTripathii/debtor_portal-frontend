...
def get_all_debtors(request):
    ...
    try:
        ...
        # Limit page size to prevent abuse (allow up to 1000 for bulk operations)
...
def update_debtor(request, account_number):
    ...
    try:
        ...
        updatable_fields = [
            ...
            'debt_type', 'original_creditor', 'case_id',
            'national_id', 'loan_contract_date'
        ]
        ...
        # Convert outstanding_balance to float if provided
        if 'outstanding_balance' in update_doc:
            try:
                update_doc['outstanding_balance'] = float(update_doc['outstanding_balance'])
            except (ValueError, TypeError):
                return JsonResponse({'error': 'Invalid outstanding_balance value'}, status=400)
        ...
        if result.matched_count > 0:
            # Fetch and return updated debtor
            updated_debtor = debtors.find_one({'account_number': account_number}, {'_id': 0})
            
            # Convert datetime to ISO format
            if updated_debtor and 'updated_at' in updated_debtor:
                updated_debtor['updated_at'] = updated_debtor['updated_at'].isoformat()
            if updated_debtor and 'created_at' in updated_debtor and updated_debtor['created_at']:
                updated_debtor['created_at'] = updated_debtor['created_at'].isoformat()
...
@csrf_exempt
@require_http_methods(["POST"])
def bulk_delete_debtors(request):
    ...
    try:
        ...
        if not account_numbers or not isinstance(account_numbers, list):
            return JsonResponse({'error': 'Invalid request. Provide account_numbers array'}, status=400)

        debtors = get_debtors_collection()
        
        # Delete debtors
        result = debtors.delete_many({'account_number': {'$in': account_numbers}})
        
        # Also delete associated images from debtor_images collection
        try:
            debtor_images = get_debtor_images_collection()
            debtor_images.delete_many({'account_number': {'$in': account_numbers}})
        except:
            pass  # Silently fail if images collection doesn't exist
...
def get_debtor_image(request, account_number):
    ...
    try:
        ...
        # FIRST: Check debtors collection for qr_code_storage_key (primary location)
        debtors = get_debtors_collection()
        debtor_doc = debtors.find_one({'account_number': account_number})
        
        print(f"Debtor doc found: {debtor_doc is not None}")
        if debtor_doc:
            print(f"qr_code_storage_key: {debtor_doc.get('qr_code_storage_key')}")
            print(f"qr_code_url: {debtor_doc.get('qr_code_url')}")
        
        if debtor_doc and debtor_doc.get('qr_code_storage_key'):
            storage_key = debtor_doc.get('qr_code_storage_key')
            print(f"Attempting to fetch QR from storage key: {storage_key}")
            try:
                # Check if file exists first
                if file_exists(storage_key):
                    print(f"File exists at: {storage_key}")
                    image_bytes = get_file(storage_key)
                    print(f"Successfully fetched {len(image_bytes)} bytes")
                    ...
                    print(f"File does not exist at: {storage_key}")
            except Exception as e:
                print(f"Error fetching QR from debtors collection: {str(e)}")
                import traceback
                traceback.print_exc()
                # Continue to fallback

        # SECOND: Try using the imported fetch_debtor_image_data function
        if debtor_doc:
            print(f"Trying fetch_debtor_image_data helper function...")
            try:
                image_bytes = fetch_debtor_image_data(account_number)
                if image_bytes:
                    print(f"Successfully fetched from helper: {len(image_bytes)} bytes")
                    image_data = base64.b64encode(image_bytes).decode('utf-8')
                    return JsonResponse({
                        'success': True,
                        'image': {
                ...
                print(f"Helper function failed: {str(e)}")

        # THIRD: Check debtor_images collection (fallback)
        debtor_images = get_debtor_images_collection()
        image_doc = debtor_images.find_one({'account_number': account_number})
        
        print(f"Image doc found in debtor_images: {image_doc is not None}")
        if image_doc:
            ...
            if storage_key or filename:
                try:
                    if storage_key:
                        ...
                        image_key = get_image_key(account_number, ext)
                        image_bytes = get_file(image_key)
...
def get_debtor_image_public(request, account_number):
    ...
    try:
        print(f"=== PUBLIC get_debtor_image for account: {account_number} ===")
        
        # FIRST: Check debtors collection for qr_code_storage_key (primary location)
        debtors = get_debtors_collection()
        debtor_doc = debtors.find_one({'account_number': account_number})
        
        print(f"Debtor found: {debtor_doc is not None}")
        if debtor_doc:
            print(f"qr_code_storage_key: {debtor_doc.get('qr_code_storage_key')}")
        
        if debtor_doc and debtor_doc.get('qr_code_storage_key'):
            storage_key = debtor_doc.get('qr_code_storage_key')
            print(f"Attempting to fetch from storage key: {storage_key}")
            try:
                if file_exists(storage_key):
                    image_bytes = get_file(storage_key)
                    print(f"Successfully fetched {len(image_bytes)} bytes")
        ...
        # SECOND: Try using helper function
        if debtor_doc:
            try:
                image_bytes = fetch_debtor_image_data(account_number)
                if image_bytes:
                    print(f"Helper function success: {len(image_bytes)} bytes")
                    image_data = base64.b64encode(image_bytes).decode('utf-8')
                    return JsonResponse({
                        'success': True,
                        'image': {
...
# ============================================================================
# S3 PRE-SIGNED URL ENDPOINTS FOR EXCEL UPLOAD
# ============================================================================

@csrf_exempt
@require_http_methods(["POST"])
def get_excel_upload_url(request):
    """Generate S3 pre-signed URL for Excel file upload"""
    import logging
    ...
    try:
        ...
        operation = data.get('operation', 'delete')  # 'delete' or 'update'
        
        # Generate unique file key
        timestamp = datetime.utcnow().strftime('%Y%m%d%H%M%S')
        file_key = f"excel-uploads/{operation}/{timestamp}-{filename}"
        ...
        # Generate pre-signed URL (5 minutes expiry)
        ...
        logger.info(f"Generated pre-signed URL for {filename}, key: {file_key}")
...
@csrf_exempt
@require_http_methods(["POST"])
def process_excel_from_s3(request):
    """Process bulk operations from Excel file in S3"""
    import logging
    logger = logging.getLogger(__name__)
    ...
    try:
        ...
        logger.info(f"Processing {operation} operation from S3 file: {file_key}")
        ...
        logger.info(f"Downloaded file from S3, size: {len(file_content)} bytes")
        ...
        if operation == 'delete':
            ...
            account_numbers = df['Account Number'].dropna().astype(str).str.strip().tolist()
                ...
                try:
                    ...
                    if debtor.get('qr_code_url'):
                        try:
                            ...
                            logger.warning(f"Error deleting QR for {account_number}: {str(e)}")
            ...
            try:
                ...
                logger.warning(f"Could not delete temp file from S3: {str(e)}")
        ...
        logger.error(f"Error processing Excel from S3: {str(e)}")
        return JsonResponse({'error': str(e)}, status=500)
